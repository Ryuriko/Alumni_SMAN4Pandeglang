...
<div id="content" class="p-4 p-md-4 pt-5 bg-secondary text-light" style="min-height: 100vh; max-height: 100vh; overflow: scroll;">
    <div class="row ps-5 pt-4">
        <div class="col-md-8 m-md-1">
            ...
            <hr class="bg-light">
            <form action="/admin/{{ $new->id }}" method="post">
                @method('put')
                @csrf
                <input type="hidden" value="{{ $new->id }}" name="id">
                <div class="mb-3">
                    <label for="title" class="form-label">Title</label>
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ $new->title }}" autofocus>
                    @error('title')
                ...
                <div class="mb-3 text-bg-light rounded-3 p-2">
                    <input id="body" class="bg-light @error('body') is-invalid @enderror" type="hidden" name="body" value="{{ $new->body }}">
